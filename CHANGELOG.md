...
### 【丢数据】restore 把 manifest 当成权威，而 park 是搬完才记账的

`tests/parking_crash.sh` 在 park 的每一条语句后 SIGKILL，然后只问一件事：**用户的东西还在不在**。四个注入点失败，最严重的一条单独复现：

```
park exit 137        ← SIGKILL，落在 mv 之后、写 manifest 之前
  a.txt 在工作区?    NO
  a.txt 在停车区?    yes
  manifest 提到?     0
restore exit 0       ← 报告成功
...
restore 遍历 `$PARK/manifest` 来决定把什么搬回去，而那份记录是**每搬完一个才写的**。崩在这个窗口里，文件停靠了却没登记，restore 直接走过去并报成功。

**调换顺序解决不了** —— 那样 manifest 会声称一个根本没搬的文件。**读目录本身没有窗口**，所以 restore 改为枚举 `$PARK/untracked`，manifest 降级为记录而非权威。

这一改顺带解开另两个注入点：崩在状态发布之后会留下一个没有 manifest 的空停车区，原来 restore 会因为重定向到不存在的文件**直接报错退出**，用户卡死；现在它清理掉并正常结束。
...
```
tests/parking_matrix.sh -> 41 passed, 0 failed   ← 照常绿
tests/parking_crash.sh  -> 4 failed
```

注入用的是 **SIGKILL**：可捕获信号会被 trap 吃掉，而“把脚本截断”是干净退出，会跑崩溃根本不会跑的 handler。两种粒度都覆盖：**两次工具调用之间挂掉**（模型的每次 Bash 调用就是一个进程，这是常态）和**某条命令执行到一半挂掉**（shim 先把文件真搬走，再杀掉调用方）。

### 一处归还的功劳

第四个失败点（崩在 tracked 的 checkout 循环中途）**不是代码缺陷，是我的判定过严** —— 回滚代码后它照样不红。那种情况下 restore 保留停车区、补丁完好、并打印 `kept $PARK`，是**对真实障碍的设计内应答**。真正被代码修复治好的是 4 条。

但这暴露了“保留停车区”可能变成万能挡箭牌，所以判定收紧成：**只有确有阻碍时才允许保留**（路径被占，或者树不完整时区里确实还存着东西）。manifest 那个 bug 正是靠“保留了并且说了”混过去的。

### shellcheck 进 CI，铉死版本

配方两块**零豁免**（它们是跑在用户未提交工作上的生产代码）。闸门自己也必须能失败：拿 v0.17.0 的 park 块去喂，**必须报错**，否则抽取器哪天坏掉会 lint 一个空文件然后永远绿。

实测：**SC2046**（不是我先前说的 SC2086）一行报出 v0.17.0 那个让带空格 tracked 文件静默丢改动、三平台都中、而 41 例全没报的 bug。

版本铉死 0.11.0：第一次接进去就红了，因为 **runner 自带 0.9.0 而我本机验的是 0.11.0** —— 两个不同的 linter，“本机干净”对 CI 什么都没说明。规则集会漂就不是一道可靠的闸门。

### 锚点静态检查

本仓库在锚点上栓过四版。`tests/check_anchors.py` 把 9 条回滚的锚点逐个在 SKILL.md 里数出现次数，**要求恰好 1 次**。套件自己一直在执行这个约束，但每次检查要跑一遍完整矩阵，所以失效锚点要一小时后才浮出来；这个一秒钟给答案，在 CI 里排在最前。验红过（故意弄坏 → exit 1）。

这一版就栓了第四次：一处被 CI 抓（那条断言用一次真实失败证明了自己是活的），另一处是我顺着查出来的 —— 改完只跑了矩阵和崩溃套件、没跑回滚套件。

### 一处主动查证

restore 不再读 manifest 之后，守它写入顺序的那条回滚会不会失去牙齿？**没有** —— 仍是 40/1，改由 `partial_case` 的 manifest/磁盘一致性断言抓到。检查换了地方，不是消失了。

### 不变

未跟踪停靠**没有**被删。我曾结论“未跟踪文件不挡 `--continue`”，**错了**，错在时机：在 rebase 之前建的文件已经是 tracked；在**冲突现场**新建的才是真未跟踪，而 `--continue` 重放后续提交时会撞车。矩阵里的 `clash` 就是这个场景（改名在途），实测删掉未跟踪半边会让 `clash / root`、`clash / sub`、`park fails partway` 三条变红 —— 护栏一直在，只是没人看得出来。已在夹具里标明。

**验证**（三平台 CI）：Ubuntu 与 macOS 停靠矩阵各 **41/0/0**、崩溃套件各 **39 passed / 0 failed / 2 skipped**、锚点 **9 条 0 失效**；逐条回滚 Ubuntu 全抓 0 跳过、macOS 全抓 1 显式跳过（BSD `mv` 不置换选项）；Windows powershell 与 pwsh 各 **28/0/0**。本机另测：POSIX 安装器矩阵 21/0/0。崩溃套件的 2 个跳过是 `||` 半句（注入的 kill 落在右支不触发），语句的完整边界都覆盖了。

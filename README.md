...
>
> **触发率实测（2026-07-30）**：claude-opus-5 / CLI 2.1.220 / 每场景 N=5，35 次无头会话。`receiving-code-review` **100%**、`spec-review` **100%**、`diagnosing-bugs` **80%**、`verification-before-completion` **60%**；正例合计 17/20 = **85%**，反例（纯问答、纯阅读）**误触 0/10**，零跳技能串触。
>
> **这些是技能调用率，不是行为达成率 —— 两个数必须一起看。** 三次“漏触”经逐条核 transcript 里的命令确认：那几次**都真跑了测试、也真定位到了 bug**，只是没调 Skill 工具。把 60% 读成“四成没验证”是错的。
>
> `resolving-merge-conflicts` 在冲突现场 5/5 **未**自动触发 —— 这是 `disable-model-invocation` 带来的**结构性保证**（探针确认它根本不出现在模型可见技能列表里），**不是模型的判断力**。
>
> 局限：macOS 未实测；只测了默认 effort；提示词全英文；判卷只看是否调用，不评调用后是否照做。复跑：`bash tests/eval/run_eval.sh --n 5` 然后 `python3 tests/eval/grade.py <results-dir>`。
>
...
0.7.0 才引入标记，旧版本装的目录没有它。安装器分不清「这是用户自己写的」和「这是我们上一版装的」，所以**默认两者都不动**并以退出码 3 告知。不加 `--adopt` 的升级只会装上全新出现的技能，已有的 8 个仍停留在旧版，改名前的 `code-review/` 也继续遮蔽内置命令 —— 退出码会说，但输出不会红。`--adopt` 表示「同名目录都算我们的」，它同时会清掉已退役的 `code-review/`。

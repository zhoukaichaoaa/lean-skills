...
**Tag every debug log** with a unique prefix, e.g. `[DEBUG-a4f2]`. Cleanup at the end becomes a single `grep -rF '[DEBUG-'` — plain `grep` would read the brackets as a character class and match half the codebase. Untagged logs survive; tagged logs die.
...
- [ ] All `[DEBUG-...]` instrumentation removed (`grep -rF '[DEBUG-'`)

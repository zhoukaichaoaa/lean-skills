...
_Based on [mattpocock/skills](https://github.com/mattpocock/skills) `implement` (MIT), rewritten: the plan from `/grill-me` is read as the spec and carried through to review; the review base is recorded as a literal SHA before the first edit; the tdd and worktree guardrails are inlined one-line (the model cannot invoke user-invoked skills, so their full versions are pointed at the user instead); review splits between `spec-review` and Claude Code's built-in `/code-review`; a re-verification step follows review fixes, because fixing a finding invalidates the evidence gathered before it; commit requires the user's say-so instead of being unconditional; exits added for a broken build and for plan drift._
